...
class HeModel:
    ...
    def setGrid(self, nx, ny):
        ...
        for patch in self.patches:
            xmin, xmax, ymin, ymax = [value for value in patch.getBoundBox()]

            dx = round((xmax - xmin) / (nx - 1))
            dy = round((ymax - ymin) / (ny - 1))
            
            yi, yf = round(ymin - dy / 2), round(ymax + 3 * dy / 4)
            xi, xf = round(xmin - dx / 2), round(xmax + 3 * dx / 4)

            for y in range(yi, yf, dy):
                for x in range(xi, xf, dx):
                    self.gridPoints.append(Point(x, y))

    def generateConnect(self):
        idArray = []
        i = 0
        ...
        idArray = np.reshape(idArray, (self.gridNx + 1, self.gridNy + 1))

        self.connect = []
        for i in range(self.gridNx + 1):
            for j in range(self.gridNy + 1):
                if idArray[i][j] != 0:
                    line = [
                        int(idArray[i+1][j]), 
                        int(idArray[i][j+1]), 
    ...
    def setTemperature(self, selectedPoints, T):
    ...
    def generateCC(self):
        ...
        for point in self.gridPoints:
            for patch in self.patches:
                if patch.isPointInside(point):
                    if point.t is None:
                        self.cc.append([0, 0.0])
                    else:
                        self.cc.append([1, float(point.t)])
    ...
    def saveJSON(self):
        ...
        print(data)
        with open('mdfInput.json', 'w') as file:
