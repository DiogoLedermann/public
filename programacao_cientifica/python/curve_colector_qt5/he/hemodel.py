...
class HeModel:
    ...
    def generateGrid(self):
        ...
        for patch in self.patches:
            xmin, xmax, ymin, ymax = [round(value) for value in patch.getBoundBox()]
            
            for x in range(xmin, xmax + self.Dx, self.Dx):
                for y in range(ymin, ymax + self.Dy, self.Dy):
                    pt = Point(x, y)
                    
                    if patch.isPointInside(pt):
                        self.gridPoints.append([x, y])
        
        data = {
            'gridPoints' : self.gridPoints
        }

        with open('grid.json', 'w', encoding='utf-8') as file:
