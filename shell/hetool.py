...
class Hetool:
    ...
    def __init__(self):
        self.__hemodel = HeModel()
        self.__heview = HeView(self.__hemodel)
        self.__hecontroller = HeController(self.__hemodel)

    def getHecontroller(self):
        return self.__hecontroller

    def getHeView(self):
        return self.__heview
...
# Input data:
#           - _hecontroller: Controller that can be obtained from the class
#                            Hetool (class Hecontroller);
...
# added or not.
def API_insertPoint(_hecontroller, _pt, _tol=0.01):
    try:
        _hecontroller.insertPoint(_pt, _tol)
        return True
    except:
        return False
...
# added or not.
def API_insertSegment(_hecontroller, _segment, _tol=0.01):
    try:
        _hecontroller.insertSegment(_segment, _tol)
        return True
    except:
        return False
...
#           - _hecontroller: Controller that can be obtained from the class
#                            Hetool (class Hecontroller);
def API_delSelectedEntities(_hecontroller):
    _hecontroller.delSelectedEntities()
...
#           - _hecontroller: Controller that can be obtained from the class
#                            Hetool (class Hecontroller);
def API_createPatch(_hecontroller):
    _hecontroller.createPatch()
...
#                        (True or False);
def API_selectPick(_hecontroller, _x,  _y,  _tol,  _shiftkey):
    _hecontroller.selectPick(_x,  _y,  _tol,  _shiftkey)
...
#           - _shiftkey: Boolean that determines if the shift key is pressed
#                        (True or False).
def API_selectFence(_hecontroller, _xmin, _xmax, _ymin, _ymax, _shiftkey):
    _hecontroller.selectFence(_xmin, _xmax, _ymin, _ymax, _shiftkey)
...
#                      (True or False);
def API_changePointSelect(_hecontroller, _select):
    _hecontroller.changePointSelect(_select)
...
#           - _select: indicator that determines if the element can be selected
#                      (True or False);
def API_changeSegmentSelect(_hecontroller, _select):
    _hecontroller.changeSegmentSelect(_select)
...
#           - _select: indicator that determines if the element can be selected
#                      (True or False);
def API_changePatchSelect(_hecontroller, _select):
    _hecontroller.changePatchSelect(_select)
...
def API_saveFile(_hecontroller, _filename):
    _hecontroller.saveFile(_filename)
...
def API_openFile(_hecontroller, _pathfile):
    _hecontroller.openFile(_pathfile)
...
def API_getPrototypes(_hecontroller):
    return _hecontroller.attManager.getPrototypes()
...
#                            Hetool (class Hecontroller);
# Output data: List of dict class objects
def API_getAttributes(_hecontroller):
    return _hecontroller.attManager.getAttributes()
...
def API_getAttributeByName(_hecontroller, _name):
    return _hecontroller.attManager.getAttributeByName(_name)
...
# Output data: dict class object
def API_getPrototypeByType(_hecontroller, _type):
    return _hecontroller.attManager.getPrototypeByType(_type)
...
#           - _hecontroller: Controller that can be obtained from the class
#                            Hetool (class Hecontroller);
#           - _prototype: Prototype of the attribute to be created (It can be obtained from
#                        the API_getPrototypeByType function) (dict);
...
# was created or not.
def API_addAttribute(_hecontroller, _prototype, _name):
    return _hecontroller.addAttribute(_prototype, _name)


# This function assigns or removes a given attribute to one or more selected
# geometric entities.
# Input data:
#           - _hecontroller: Controller that can be obtained from the class
#                            Hetool (class Hecontroller);
#           - _attribute: Attribute to be assigned (It can be obtained from
#                        the API_getAttributeByName function) (dict).
def API_setAttribute(_hecontroller, _attribute):
    _hecontroller.setAttribute(_attribute)
...
def API_saveAtribute(_hecontroller, _name, _values):
    _hecontroller.saveAtribute(_name, _values)
...
#           - _name: Name of attribute to be created (created attributes must
#                    have different names) (str).
def API_removeAttribute(_hecontroller, _name):
    _hecontroller.removeAttribute(_name)
...
def API_renameAttribute(_hecontroller, _oldname, _newname):
    return _hecontroller.renameAttribute(_oldname, _newname)
...
def API_getAttributeSymbol(_hecontroller, _attribute, _scale=1.0, _pt=None, _seg=None, _patch=None):
    return _hecontroller.getAttributeSymbol(_attribute, _scale, _pt, _seg, _patch)
...
# This function is responsible for managing the creation of two-dimensional meshes.
# Meshes are created only on selected faces. Use the "number of subdivisions"
# attribute on the edges of the face before creating the mesh.
# Input data:
#           - _hecontroller: Controller that can be obtained from the class
#                            Hetool (class Hecontroller);
#           - _mesh_type: The type of mesh to be created (int).
#                           - Options: 1 - Bilinear Transfinite;
#                                      2 - Trilinear Transfinite;
#                                      3 - Quadrilateral Template;
#                                      4 - Quadrilateral Seam;
#                                      5 - Triangular Boundary Contraction.
#           - _shape_type: The element format (int).
#                           - Options: 1 - Triangular;
#                                      2 - Quadrilateral.
#           - _elem_type: The type of element (int).
#                           - Options: 1 - Linear;
#                                      2 - Quadratic.
#           - _diag_type: The diagonal type (only for triangular mesh type 1) (int).
#                           - Options: 1 - Right;
#                                      2 - Left;
#                                      3 - Union Jack;
#                                      4 - Optimal.
#           - _qt_flag: boolean that determines whether the mesh will be generated
#                       by a quadtree or a regular grid (only for triangular mesh type 5)
#                       (True or False).
# Output data: returns a boolean (True or False) indicating whether all meshes were successfully
# generated
def API_generateMesh(_hecontroller, _mesh_type, _shape_type,  _elem_type, _diag_type=None, _qt_flag=False):

    if _mesh_type == 1:
        mesh_type = "Bilinear Transfinite"
    elif _mesh_type == 2:
        mesh_type = "Trilinear Transfinite"
    elif _mesh_type == 3:
        mesh_type = "Quadrilateral Template"
    elif _mesh_type == 4:
        mesh_type = "Quadrilateral Seam"
    else:
        mesh_type = "Triangular Boundary Contraction"
    ...
    if _diag_type is not None:
        if _diag_type == 1:
            diag_type = "Right"
        elif _diag_type == 2:
            diag_type = "Left"
        elif _diag_type == 3:
            diag_type = "Union Jack"
        else:
            diag_type = "Optimal"
    else:
        diag_type = None
    ...
    try:
        _hecontroller.generateMesh(
            mesh_type, shape_type,  elem_type, diag_type, _qt_flag)
        return True
    except:
        return False


# This function removes meshes from the faces that are selected.
# Input data:
#           - _hecontroller: Controller that can be obtained from the class
#                            Hetool (class Hecontroller);
def API_delSelectedMesh(_hecontroller):
    _hecontroller.delSelectedMesh()
...
# Input data:
#           - _heview: Viewer that can be obtained from the class Hetool (class Heview).
# Output data: Returns a list of objects of class Point.For more information
# about this class see src\geometry\point.
def API_getPoints(_heview):
    return _heview.getPoints()
...
def API_getSegments(_heview):
    return _heview.getSegments()
...
# about this class see src\geometry\Patch.
def API_getPatches(_heview):
    return _heview.getPatches()
...
# Output data: List of dict class objects
def API_getEntityAttributes(_heview, _entity):
    return _heview.getEntityAttributes(_entity)
...
# the user.
def API_getSelectedPoints(_heview):
    return _heview.getSelectedPoints()
...
# the user.
def API_getSelectedSegments(_heview):
    return _heview.getSelectedSegments()
...
# the user.
def API_getSelectedPatches(_heview):
    return _heview.getSelectedPatches()


# This function returns all the points present in the mesh of a patch.
# # Input data:
#           - _heview: Viewer that can be obtained from the class Hetool (class Heview);
#           - _patch: Patch class object (class Patch);For more information
#                     about this class see src\geometry\Patch;
# Output data: Returns a list of objects of class Point.
def API_getMeshPoints(_heview, _patch):
    return _heview.getMeshPoints(_patch)


# This function returns all the segments present in the mesh of a patch.
# # Input data:
#           - _heview: Viewer that can be obtained from the class Hetool (class Heview);
#           - _patch: Patch class object (class Patch);For more information
#                     about this class see src\geometry\Patch;
# Output data: Returns a list of objects of class Segment.
def API_getMeshSegments(_heview, _patch):
    return _heview.getMeshSegments(_patch)


# This function returns all the patches present in the mesh of a patch.
# # Input data:
#           - _heview: Viewer that can be obtained from the class Hetool (class Heview);
#           - _patch: Patch class object (class Patch);For more information
#                     about this class see src\geometry\Patch;
# Output data: Returns a list of objects of class Patch.
def API_getMeshPatches(_heview, _patch):
    return _heview.getMeshPatches(_patch)
...
def API_isEmpty(_heview):
    return _heview.isEmpty()
...
#                   direction (float);
def API_getBoundBox(_heview):
    return _heview.getBoundBox()
...
#                   close to the click (float or int).
def API_snapToSegment(_heview, _x, _y, _tol):
    return _heview.snapToSegment(_x, _y, _tol)
...
#                   close to the click (float or int).
def API_snapToPoint(_heview, _x, _y, _tol):
    return _heview.snapToPoint(_x, _y, _tol)
...
#                     about this class see src\geometry\point;
# Output data: Returns a list of objects of class Segment.
def API_getIncidentSegmentsFromPoint(_heview, _point):
    return _heview.getIncidentSegmentsFromPoint(_point)
...
#                     about this class see src\geometry\point;
# Output data: Returns a list of objects of class Patch.
def API_getIncidentPatchesFromPoint(_heview, _point):
    return _heview.getIncidentPatchesFromPoint(_point)
...
#                     about this class see src\geometry\point;
# Output data: Returns a list of objects of class Point.
def API_getAdjacentPointsFromPoint(_heview, _point):
    return _heview.getAdjacentPointsFromPoint(_point)
...
#                       src\geometry\segments\Polyline;
# Output data: Returns a list of objects of class Segment.
def API_getAdjacentSegmentsFromSegment(_heview, _segment):
    return _heview.getAdjacentSegmentsFromSegment(_segment)
...
#                       src\geometry\segments\Polyline;
# Output data: Returns a list of objects of class Patch.
def API_getIncidentPatchesFromSegment(_heview, _segment):
    return _heview.getIncidentPatchesFromSegment(_segment)
...
#                       src\geometry\segments\Polyline;
# Output data: Returns a list of objects of class Point.
def API_getIncidentPointsFromSegment(_heview, _segment):
    return _heview.getIncidentPointsFromSegment(_segment)
...
#                     about this class see src\geometry\Patch;
# Output data: Returns a list of objects of class Segment.
def API_getIncidentSegmentsFromPatch(_heview, _patch):
    return _heview.getIncidentSegmentsFromPatch(_patch)
...
#                     about this class see src\geometry\Patch;
# Output data: Returns a list of objects of class Patch.
def API_getAdjacentPatchesFromPatch(_heview, _patch):
    return _heview.getAdjacentPatchesFromPatch(_patch)
...
#                     about this class see src\geometry\Patch;
# Output data: Returns a list of objects of class Point.
def API_getIncidentPointsFromPatch(_heview, _patch):
    return _heview.getIncidentPointsFromPatch(_patch)
...
#                     about this class see src\geometry\Patch;
# Output data: Returns a list of objects of class Patch.
def API_getInternalPacthesFromPatch(_heview, _patch):
    return _heview.getInternalPacthesFromPatch(_patch)
...
# Note 1: To use this function efficiently, get all patches by API_getPatches function
...
def API_tesselate(_patch):
    pts = _patch.getPoints()
    return Tesselation.tesselate(pts)
...
class Line(Segment):
    ...
    def setParentId(self, _parentId):
...
class Patch:
    ...
    def addTopConnection(self, _patch):
        self.topConnections.append(_patch)

    def addBottomConnection(self, _patch):
        self.bottomConnections.append(_patch)

    def setRegion(self, _region):
        self.region = _region
...
class HeFile():
    ...
    def saveFile(_shell, _attributes, _filename):
        ...
        for face in faces:
            ...
            attributes = face.patch.attributes
            att_list = []
            for att in attributes:
                if att['type'] != "Mesh":
                    att_list.append(att['name'])
...
class AttribManager:
    ...
    def __init__(self):
        ...
        for prototype in self.prototypes:
            values_types = []
            for key in prototype['properties']:
                valueType = type(prototype['properties'][key])
                if valueType == int:
                    ...
                    if len(prototype['properties'][key]) == 3:
                        values_types.append("color")
                    else:
                        values_types.append("list")
                elif valueType == str:
                    values_types.append("string")

            # creates a new key
            prototype['properties_type'] = values_types.copy()
...
class AttribSymbols:
    ...
    def getSymbol(_attribute, _scale, _pt=None, _seg=None, _patch=None):
        ...
        # ajust color for canvas color scale
        try:
            # try to get a color
            color = []
            for item in _attribute['properties']['Color']:
    ...
    def supportSegment(_attribute, _seg, _scale):
        ...
        squares = []
        circles = []
        points = _seg.getPoints()

        for att in _seg.attributes:
            if att['type'] == 'Number of Subdivisions':
                pts = AttribSymbols.Nsbdvs(att, _seg)
                points = pts
                break
    ...
    def Nsbdvs(_attribute, _seg):
        points = []
        properties = _attribute['properties']
        number = properties['Number Subdv']

        if number == 0:
            return points

        seg_pts = _seg.getPoints().copy()
        while len(seg_pts) >= 2:
            aux_line = Line(seg_pts[0], seg_pts[1])
            for i in range(1, number):
                pt = aux_line.getPoint(i/number)
                points.append(pt)

            if len(seg_pts) != 2:
                points.append(aux_line.getPoint(1))
...
class HeController:
    ...
    def setAttribute(self, _attribute):
        ...
        if _attribute['applyOnVertex']:
            points = self.hemodel.getPoints()

            for pt in points:
                if pt.isSelected():
                    pt.setSelected(False)
                    setAtt = SetAttribute(pt, _attribute)
                    setAtt.execute()
                    self.undoredo.insertOperation(setAtt)

        if _attribute['applyOnEdge']:
            segments = self.hemodel.getSegments()

            for seg in segments:
                if seg.isSelected():
                    seg.setSelected(False)
                    setAtt = SetAttribute(seg, _attribute)
                    setAtt.execute()
                    self.undoredo.insertOperation(setAtt)

                    # change the support conditions of the segment points
                    if _attribute['type'] == 'Support Conditions':
                        seg_vertices = seg.edge.incidentVertices()
                        if _attribute not in seg_vertices[0].point.attributes:
                            setAtt = SetAttribute(
                                seg_vertices[0].point, _attribute)
                            setAtt.execute()
                            self.undoredo.insertOperation(setAtt)

                        if _attribute not in seg_vertices[1].point.attributes:
                            setAtt = SetAttribute(
                                seg_vertices[-1].point, _attribute)
                            setAtt.execute()
                            self.undoredo.insertOperation(setAtt)

        if _attribute['applyOnFace']:
            patches = self.hemodel.getPatches()

            for patch in patches:
                if patch.isSelected() and not patch.isDeleted:
                    patch.setSelected(False)
                    setAtt = SetAttribute(patch, _attribute)
                    setAtt.execute()
                    self.undoredo.insertOperation(setAtt)
